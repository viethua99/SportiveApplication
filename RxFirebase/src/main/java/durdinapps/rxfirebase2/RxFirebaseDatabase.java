...
public class RxFirebaseDatabase {
    ...
    @NonNull
    public static Maybe<DataSnapshot> observeExistSingleValueEvent(@NonNull final Query query) {
        return Maybe.create(new MaybeOnSubscribe<DataSnapshot>() {
            @Override
            public void subscribe(final MaybeEmitter<DataSnapshot> emitter) throws Exception {
                query.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if (emitter.isDisposed()){
                            Log.e(TAG, "observeExistSingleValueEvent return ");
                            return;
                        }
                        emitter.onSuccess(dataSnapshot);
                    }
    ...
    /**
     * Set the given value on the specified key {@link DatabaseReference}.
     *
     * @param ref   reference represents a particular location in your database.
     * @param value value to update.
     * @return a {@link Completable} which is complete when the set value return key of object
     */
    public static Observable<String> setValueWithKey(@NonNull final DatabaseReference ref,
        ...
        return Flowable.create(new FlowableOnSubscribe<RxFirebaseChildEvent<DataSnapshot>>() {
            ...
            public void subscribe(final FlowableEmitter<RxFirebaseChildEvent<DataSnapshot>> emitter) throws Exception {
                ...
                emitter.setCancellable(new Cancellable() {
                    @Override
                    public void cancel() throws Exception {
                        Log.e(TAG, "observeChildEvent() ->>>>>>>  removeEventListener <<<<<<---");
    ...
    @NonNull
    public static <T> Maybe<T> observeSingleValueEventWithoutFilterExistData(@NonNull final Query query,
                                                       @NonNull final Function<? super DataSnapshot, ? extends T> mapper) {
        return observeExistSingleValueEvent(query)
                .map(mapper);
    }
    ...
    /**
     * remove the specific child key
     *
     * @param ref        reference represents a particular location in your database.
     * @return a {@link Completable} which is complete when the update children call finish successfully.
     */
    @NonNull
    public static Completable removeChildren(@NonNull final DatabaseReference ref) {

        return Completable.create(new CompletableOnSubscribe() {
            @Override
            public void subscribe(final CompletableEmitter emitter) throws Exception {
                RxCompletableHandler.assignOnTask(emitter, ref.removeValue());
            }
        });
    }
